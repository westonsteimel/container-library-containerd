...
version=`curl --silent "https://api.github.com/repos/containerd/containerd/releases/latest" | jq .tag_name | xargs`
revision=`curl --silent "https://api.github.com/repos/containerd/containerd/commits/${version}" | jq .sha | xargs`
version=${version#"v"}
